...
def test_bundle_exposes_confirmed_path_free_repository_registration() -> None:
    ...
    required_strings = (
        ...
        "$HERMES_HOME/repository-registration-defaults.yaml",
        "$HERMES_HOME/projects/<project-id>/registration.yaml",
        "$HERMES_HOME/projects/<project-id>/credential-bindings.yaml",
        "<project-id> is Daidala's stable repository identifier",
        "it is not a GitHub Project",
        "Daidala derives the project ID from its GitHub owner and repository name",
        "It becomes effective after a maintainer reviews and merges that pull request",
        "When a maintainer authors .daidala/project.yaml manually",
        "changing it after registration has no supported migration path",
        "A GitHub Project is optional",
        "registration neither requires nor creates one",
        "presentation/intake metadata",
        "Registration-specific values are immutable",
        ...
        "GitHub usernames",
        "github.com/example-operator",
        "Do not use an email address",
        "Hermes profile name",
        "Allowlist copied onto every new registration",
        "This is not dashboard plan approval",
