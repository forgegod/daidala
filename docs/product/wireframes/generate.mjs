...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; } body { margin:0; width:1440px; min-height:960px; overflow:hidden; background:var(--bg); color:var(--text); font:14px/1.4 Inter,ui-sans-serif,system-ui,sans-serif; }
    ...
    .workspace { padding:32px 34px; } .crumb { color:var(--amber); font-size:11px; letter-spacing:.1em; text-transform:uppercase; } .intro { max-width:800px; color:var(--muted); } .card { max-width:1060px; padding:24px; border:1px solid var(--line); background:var(--panel); } .field { display:grid; grid-template-columns:1fr 164px; gap:10px; margin:20px 0 10px; } input { padding:13px; border:1px solid var(--line); background:#001a18; color:var(--cream); } button { border:1px solid var(--cream); background:var(--cream); color:var(--ink); font-weight:800; letter-spacing:.06em; text-transform:uppercase; } .hint { color:var(--muted); font-size:12px; }
...
<body>
  <div class="profile">Managing profile “demo-controller” — repository registration writes only to this profile after confirmation.</div>
  ...
  <main><header><h1>Daidala</h1><div class="tabs"><div class="tab">Workflows</div><div class="tab">Artifacts</div><div class="tab active">Config</div></div></header>
    <section class="workspace"><p class="crumb">Config / Repositories</p><h2>Register a GitHub repository</h2><p class="intro">Paste one GitHub.com repository link. Daidala inspects committed policy before it writes exactly two non-secret profile-local records.</p>
      <section class="card"><h3>Repository identity</h3><div class="field"><input value="github.com/acme/payments-service" aria-label="GitHub repository link"><button>Inspect repository</button></div><p class="hint">Supported: GitHub repository pages and clone URLs. No paths, profiles, credential aliases, or tokens are browser inputs.</p>
        <div class="preview"><section><p class="label">Repository identity</p><p class="value">acme/payments-service</p><p class="label">Project ID</p><p class="value">acme-payments-service</p><p class="label">Manifest digest</p><code>6f12…9c80</code></section><section><p class="label">Mounted controller readiness</p><p class="ok">✓ Board selected</p><p class="ok">✓ Attended target configured</p><p class="warn">! Delivery secret source not checked</p><p class="hint">Delivery readiness does not grant commit or push authority.</p></section></div>
