...
const SCREENS = [
  ...
  },
  {
    ...
    viewport: { width: 1440, height: 2040 },
  },
  {
    capability: "CAP-0005",
...
function buildOperatorDashboardValidationHtml(screen) {
  return `<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>${screen.capability} — ${screen.title}</title><style>
:root{color-scheme:dark;--bg:#001f1d;--panel:#032725;--line:#244845;--text:#f4ead8;--muted:#8fa6a1;--cream:#ffe6ca;--amber:#ffc933;--red:#ff786f;--blue:#5688ff}*{box-sizing:border-box}body{margin:0;width:1440px;height:960px;overflow:hidden;background:var(--bg);color:var(--text);font:14px/1.4 Inter,ui-sans-serif,system-ui,sans-serif}.shell{display:grid;grid-template-columns:240px 1fr;height:960px}aside{padding:18px 15px;border-right:1px solid var(--line)}.brand{font-size:19px;font-weight:800;line-height:1;letter-spacing:.08em}nav{display:grid;gap:3px;margin-top:25px}nav div{padding:7px 11px;color:#b8c4c0;font-size:11px;letter-spacing:.13em;text-transform:uppercase}nav .active{background:#29413f;color:var(--cream);font-weight:800}main{min-width:0}header{height:72px;padding:18px 28px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid var(--line)}h1,h2,h3,p{margin-top:0}h1{margin:0;font-size:18px;letter-spacing:.09em}.views{display:flex}.view{padding:10px 20px;border:1px solid var(--line);color:var(--muted);text-transform:uppercase;font-size:11px;letter-spacing:.1em}.view.active{color:#102321;background:var(--cream);border-color:var(--cream);font-weight:800}.workspace{padding:26px 34px}.eyebrow{margin-bottom:5px;color:var(--blue);font-size:10px;font-weight:800;letter-spacing:.12em;text-transform:uppercase}.intro{max-width:760px;color:var(--muted)}.wizard{max-width:930px;margin-top:22px;padding:22px;border:1px solid var(--line);border-radius:8px;background:var(--panel)}.steps{display:flex;gap:7px;margin:17px 0}.step{padding:6px 10px;border:1px solid var(--line);border-radius:999px;color:var(--muted);font-size:10px;text-transform:uppercase}.step.active{border-color:var(--cream);color:var(--cream)}.grid{display:grid;grid-template-columns:1fr 1fr;gap:14px}.field{display:block;color:var(--muted);font-size:10px;font-weight:800;letter-spacing:.1em;text-transform:uppercase}input,select{width:100%;margin-top:7px;padding:11px 12px;border:1px solid var(--line);border-radius:6px;background:#001a18;color:var(--cream);font:inherit}.invalid{border-color:var(--red)}.error{margin:7px 0 0;padding:9px 10px;border:1px solid var(--red);border-radius:6px;background:#321e1d;color:var(--red);font-size:12px;text-transform:none;letter-spacing:normal}.actions{display:flex;gap:9px;margin-top:22px}button{padding:11px 14px;border:1px solid var(--line);border-radius:6px;background:transparent;color:var(--text);font-weight:800}button:disabled{opacity:.45;color:var(--muted)}.note{margin-top:17px;color:var(--muted);font-size:12px}.annotation{position:fixed;right:14px;bottom:11px;color:#617b77;font-size:10px}</style></head>
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; } body { margin:0; width:1440px; min-height:2040px; overflow:hidden; background:var(--bg); color:var(--text); font:14px/1.4 Inter,ui-sans-serif,system-ui,sans-serif; }
...
<body>
  <div class="profile">Every existing Hermes profile is listed with its registered workspace tuple. Board state and optional GitHub Project link are path-free.</div>
  ...
  <main><header><h1>Daidala</h1><div class="tabs"><div class="tab">Workflows</div><div class="tab">Artifacts</div><div class="tab active">Config</div></div></header>
    <section class="workspace"><p class="crumb">Config / GitHub Repositories</p><h2>Register a GitHub repository</h2><p class="intro">Every existing Hermes profile is visible at once. A registered row shows its repository, slug, board status, and optional GitHub Project link. Daidala inspects committed policy before it binds or creates the board and writes two non-secret profile-local records.</p>
      <section class="card"><h3>Profile inventory</h3>
        <div class="profiles">
          <article class="profile-card"><h3>demo-controller</h3><div class="facts"><div><span>Repository</span><b>acme/payments-service</b></div><div><span>Slug</span><b>acme-payments-service</b></div><div><span>Board</span><b>daidala-acme-payments-service · bound</b></div><div><span>GitHub Project</span><b>not linked</b></div></div><p class="hint">Registration defaults are configured.</p><div class="field"><input value="github.com/acme/payments-service" aria-label="GitHub repository link for demo-controller"><button>Inspect repository</button></div><p class="hint">Register another repository</p><div class="field"><input placeholder="github.com/owner/repository" aria-label="Register another repository for demo-controller"><button>Inspect repository</button></div></article>
          <article class="profile-card"><h3>example-controller</h3><p class="hint">Registration defaults are not configured for this profile.</p><button>Configure registration defaults</button><p class="hint">No repository registered</p><div class="field"><input placeholder="github.com/owner/repository" aria-label="GitHub repository link for example-controller"><button>Inspect repository</button></div>
            <div class="confirm"><p class="hint">These values become the profile defaults copied onto every new repository registration. Enter aliases and environment-variable names only. GitHub token values stay in the process environment.</p><div class="defaults"><p class="hint">Configured GitHub access rights</p><label>Intake alias<small class="field-help">Logical name for the already-configured read-only GitHub access used to list and claim issues. Lowercase slug such as github-read-issues. Must differ from the findings alias. This is a label, not the GitHub token.</small><input placeholder="github-read-issues" aria-label="Intake alias"></label><label>Intake environment variable<small class="field-help">Name of the process environment variable that already holds the intake GitHub token, such as EXAMPLE_GITHUB_INTAKE_TOKEN. Use a classic personal access token. Mandatory classic scopes are read:project and read:org.</small><input placeholder="EXAMPLE_GITHUB_INTAKE_TOKEN" aria-label="Intake environment variable"></label><label>Maintainers<small class="field-help">Allowlist copied onto every new registration. Only these GitHub usernames may mark an issue ready and admit a cycle; the same list authorizes claim comments and notification prerequisite evidence. This is not dashboard plan approval. Enter the login in the profile URL, such as example-operator for github.com/example-operator. Do not use an email address, display name, Hermes profile name, or Git author name.</small><input placeholder="example-operator" aria-label="Maintainers"></label><label>Active cycles<small class="field-help">Declared cycle concurrency copied onto every new registration. v1 requires exactly 1.</small><input value="1" aria-label="Active cycles"></label></div><label><input type="checkbox"> I confirm writing these registration defaults</label><button>Save registration defaults</button></div></article>
        </div>
        <div class="preview"><section><p class="label">Repository identity</p><p class="value">acme/payments-service</p><p class="label">Project ID</p><p class="value">acme-payments-service</p><p class="label">Manifest digest</p><code>6f12…9c80</code></section><section><p class="label">Workspace board</p><p class="ok">✓ daidala-acme-payments-service · create</p><p class="ok">✓ Attended target configured</p><p class="warn">! Credential not available</p><p class="hint">Credential readiness does not grant commit or push authority.</p></section></div>
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; } body { margin:0; width:1440px; min-height:960px; overflow:hidden; background:var(--bg); color:var(--text); font:14px/1.4 Inter,ui-sans-serif,system-ui,sans-serif; }
    ...
    .shell { display:grid; grid-template-columns:268px 1fr; min-height:922px; } aside { padding:22px 16px; border-right:1px solid var(--line); } .brand { font-size:19px; font-weight:800; letter-spacing:.12em; } nav { margin-top:30px; display:grid; gap:4px; } nav div { padding:9px 12px; color:var(--muted); font-size:11px; letter-spacing:.1em; text-transform:uppercase; } nav .active { color:var(--cream); background:#29413f; font-weight:800; }
    main { min-width:0; } header { height:86px; padding:24px 34px; display:flex; align-items:center; justify-content:space-between; border-bottom:1px solid var(--line); } h1,h2,h3,p { margin-top:0; } h1 { font-size:20px; letter-spacing:.08em; text-transform:uppercase; } .tabs { display:flex; } .tab { padding:11px 22px; border:1px solid var(--line); color:var(--muted); font-size:11px; letter-spacing:.1em; text-transform:uppercase; } .tab.active { color:var(--ink); background:var(--cream); border-color:var(--cream); font-weight:800; }
    .workspace { padding:32px 34px; } .crumb { color:var(--amber); font-size:11px; letter-spacing:.1em; text-transform:uppercase; } .intro { max-width:820px; color:var(--muted); } .card { max-width:1060px; padding:24px; border:1px solid var(--line); background:var(--panel); } .field { display:grid; grid-template-columns:1fr 228px; gap:10px; margin:20px 0 10px; } input,select { padding:13px; border:1px solid var(--line); background:#001a18; color:var(--cream); } button { border:1px solid var(--cream); background:var(--cream); color:var(--ink); font-weight:800; letter-spacing:.06em; text-transform:uppercase; } .hint { color:var(--muted); font-size:12px; }
    .preview { display:grid; grid-template-columns:1fr 1fr; gap:1px; margin-top:24px; border:1px solid var(--line); background:var(--line); } .preview section { min-height:168px; padding:18px; background:var(--soft); } .label { color:var(--muted); font-size:10px; letter-spacing:.11em; text-transform:uppercase; } .value { margin:5px 0 16px; color:var(--cream); font-size:17px; font-weight:700; } .ok { color:var(--green); } .warn { color:var(--amber); } code { color:var(--green); font-size:12px; } .confirm { margin-top:22px; padding:16px; border:1px solid var(--amber); background:#182c28; } .confirm label { display:block; margin-bottom:12px; color:var(--cream); } .confirm button { padding:12px 20px; } .note { margin-top:20px; max-width:1060px; padding:14px 16px; border:1px dashed #496763; color:var(--muted); } .annotation { position:fixed; right:14px; bottom:11px; color:#617b77; font-size:10px; }
  </style>
</head>
<body>
  ...
  <div class="shell"><aside><div class="brand">HERMES<br>AGENT</div><nav><div>Chat</div><div>Sessions</div><div>Kanban</div><div>Skills</div><div>Plugins</div><div class="active">Daidala</div></nav></aside>
  <main><header><h1>Daidala</h1><div class="tabs"><div class="tab">Workflows</div><div class="tab">Artifacts</div><div class="tab active">Config</div></div></header>
    <section class="workspace"><p class="crumb">Config / GitHub Repositories</p><h2>Bootstrap Daidala policy</h2><p class="intro">When a repository has no committed .daidala/project.yaml, the inspect control becomes Apply default policy. Confirm, publish conservative policy on branch chore/daidala-bootstrap-project-policy, then open the compare/PR link. That link stays on this row after a Hermes restart until the repository is registered.</p>
  ...
  <div class="shell">
    ...
    <main class="main">
      ...
      <section class="workspace">
        ...
        <section class="guidance"><div><b>Workflow supervision</b><p>Use Workflows to inspect approval-gated progress and make the next required human decision from source-bound evidence.</p></div><div class="advice"><b>Start workspace</b><p>Registered GitHub repository · bound board</p><p>Not selectable · uniqueness reason and conclusion</p><p>Existing unregistered repository · clean Git board</p><p>Initialize local project · policy and initial commit</p><div class="button">Start workflow</div></div></section>
