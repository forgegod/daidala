...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; } body { margin:0; width:1440px; min-height:1480px; overflow:hidden; background:var(--bg); color:var(--text); font:14px/1.4 Inter,ui-sans-serif,system-ui,sans-serif; }
      ...
      <section class="card"><h3>Profile inventory</h3>
        <div class="profiles">
          ...
          <article class="profile-card"><h3>example-controller</h3><p class="hint">Registration defaults are not configured for this profile.</p><button>Configure registration defaults</button><p class="hint">No repository registered</p><div class="field"><input placeholder="github.com/owner/repository" aria-label="GitHub repository link for example-controller"><button>Inspect repository</button></div>
            <div class="confirm"><p class="hint">These values become the profile defaults copied onto every new repository registration. Enter aliases and environment-variable names only. Token values stay in the process environment.</p><div class="defaults"><label>Intake alias<small class="field-help">Logical name for the GitHub read credential used to list and claim issues. Lowercase slug such as github-read-issues. Must differ from the findings alias. This is a label, not the token.</small><input placeholder="github-read-issues" aria-label="Intake alias"></label><label>Notification destination<small class="field-help">Where Hermes sends attended reviews. Must be an explicit non-home target such as telegram:&lt;chat-id&gt; or telegram:&lt;chat-id&gt;:&lt;thread-id&gt;. Do not use home.</small><input placeholder="telegram:-1000000000000:1" aria-label="Notification destination"></label><label>Active cycles<small class="field-help">Declared cycle concurrency copied onto every new registration. v1 requires exactly 1.</small><input value="1" aria-label="Active cycles"></label></div><label><input type="checkbox"> I confirm writing these registration defaults</label><button>Save registration defaults</button></div></article>
        </div>
