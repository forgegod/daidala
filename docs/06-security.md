...
flowchart LR
    ...
    H -->|"loads Python package in-process"| W["Daidala plugin code"]
    ...
    H -->|"explicit skill load"| S["Read-only daidala:orchestrate resource"]
```
...
skills. Daidala authorization comes from the persisted manifest checked at the
evidence boundary.

AI-DLC v1.0.1 is pinned to its upstream commit but ships editor rule files,
not Hermes skills. Daidala therefore packages a small MIT-0-attributed
...
Daidala requires no credentials. Do not add credentials to pack YAML,
...
Daidala makes no model API call and therefore neither meters tokens nor owns a
...
- no Daidala server, nested Hermes chat process, or private Hermes database
...
- Registration: `daidala/__init__.py`
- Pack loading and validation: `daidala/packs.py`
