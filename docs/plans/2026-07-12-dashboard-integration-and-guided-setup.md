...
> `daidala/AGENTS.md`, `tests/AGENTS.md`, this plan, the current official Hermes
...
Make Daidala easier to adopt and operate through a guided setup skill and,
only if the supported-host probe succeeds, a complementary dashboard surface:

1. an officially distributed `daidala:setup` skill that guides installation,
...
dropped. The dashboard is a presentation and command surface over Daidala and Hermes. It
...
Daidala already has the difficult runtime guarantees: a Kanban-native card
...
1. prove how the supported Hermes host discovers dashboard assets and backend
   routes;
2. ship `daidala:setup` as the smallest immediately useful onboarding path;
...
Implemented Daidala behavior already provides:

- `daidala_pack_info`, `daidala_start`, `daidala_status`,
  `daidala_replace_constraints`, `daidala_approve`, and cancellation and
  evidence tools;
- one explicit named board and a complete executable-stage profile map per
  workflow;
- live Kanban status combined with Daidala policy facts without status
  mirroring;
...
- a bundled `daidala:orchestrate` worker skill.
...
Daidala's existing Git-directory installation, but it has not been exercised
against Daidala's pinned Hermes host and therefore remains a Phase 0 gate.
...
| Pack, policy, approval, artifact, worktree, and evidence integrity | Existing Daidala service and ledger |
| Setup guidance and decisions about user intent | `daidala:setup` skill, with explicit confirmation before mutation |
| Dashboard rendering and local form state | Daidala dashboard UI plugin |
| Dashboard data and mutation boundary | Thin backend adapter calling existing Daidala services and documented Hermes operations |
| Next-action recommendation | Pure deterministic projection from current Daidala policy facts and live Kanban state |
...
A user loads `daidala:setup`. The skill:
...
6. previews the complete `daidala_start` input and asks for confirmation;
7. starts the workflow through the existing tool and points the user to Kanban or
   the Daidala dashboard tab.
...
The `/daidala` tab has four focused views:
...
A small page-scoped slot may show the number of pending Daidala decisions on a
built-in Hermes page. Use augmentation, not `tab.override`; Daidala must not own
...
| 1. Official setup skill | Done | Add and register `daidala:setup`, preserving the current start schema and human confirmation boundary. | `198 passed in 16.24s`; fresh-process `daidala:setup` load passed. |
...
| 3. Read-only dashboard UI | Done | Add the Daidala tab, workflow/progress cards, decisions view, and a pending-decision slot. | Isolated Hermes v0.18.2 desktop/narrow browser states and `sessions:top` slot passed; dashboard tests passed. |
...
1. install or link Daidala using the same directory-plugin shape used by the
   release probe;
2. place a minimal `dashboard/manifest.json`, IIFE bundle, stylesheet, and
   `plugin_api.py` beneath the Daidala plugin directory;
3. start the existing Hermes dashboard on a non-default local test port;
4. verify `/api/dashboard/plugins` discovers Daidala and serves its static
   bundle;
5. verify the bundle registers one tab and one page-scoped slot through the SDK;
6. verify `/api/plugins/daidala/health` is mounted after process startup;
...
- preferred: package assets and have Daidala's existing dry-run/apply setup
...
- fallback: distribute a separately installable, version-locked dashboard
  directory and have `daidala:setup` guide its installation.
...
`$HERMES_HOME/plugins/daidala/dashboard/manifest.json`, served its manifest,
IIFE, and stylesheet, registered `/daidala`, rendered a `sessions:top` slot,
...
no `kanban:top` slot, so Daidala will use `sessions:top` for the compact pending-
decision augmentation and retain its normal `/daidala` tab. A pip entry point
does not materialize dashboard files into the profile plugin directory, so the
single installation contract is to package the assets and extend Daidala's
...
## Phase 1 — official `daidala:setup` skill
...
- `daidala/skills/setup/SKILL.md`
- optional focused references under `daidala/skills/setup/references/`
- `plugin.yaml`
...
- Register the bundled skill as `daidala:setup` and declare it in
  `plugin.yaml`.
- Keep setup and orchestration separate: setup prepares and starts; workers still
  use `daidala:orchestrate`.
- Use `daidala_pack_info`, existing Hermes status/board/profile surfaces, and
  `daidala_start`; do not encode another copy of pack data.
...
- Publish the skill with Daidala's normal release artifact; “official” means
  versioned, tested, documented, and distributed by the Daidala project rather
  than created in a user's mutable local skill store.

### Gate

A fresh-process plugin probe loads `skill_view("daidala:setup")`; package-content
...
produces the same `daidala_start` request when the dashboard is unavailable.
...
- `daidala/recommendations.py` for pure derivation, if keeping it in the router
...
`daidala/recommendations.py` owns the closed action vocabulary and pure
...
- Register a normal `/daidala` tab after the native Kanban tab when supported.
...
- Render progress from live card statuses and policy identity from Daidala.
...
The dependency-free IIFE registers `/daidala` and `sessions:top` through SDK
...
Turn Daidala's required inputs into a short visual flow without hiding policy.
...
Daidala or Hermes operations. It does not accept arbitrary commands.
...
- Surface the server's exact limits: at most 16 global and 16 per-phase items,
  ...
  validation.
- Validate on demand through `daidala/constraints.py`, not duplicated
  JavaScript rules.
...
- Add an optional restrained Daidala dashboard theme only if it improves setup
...
The release artifact must load both `daidala:setup` and
`daidala:orchestrate`, expose the documented dashboard assets through the chosen
...
- `daidala/__init__.py`
- `daidala/service.py`
- `daidala/recommendations.py` (new)
- `daidala/skills/setup/SKILL.md` (new)
...
- a standalone Daidala dashboard or HTTP service;
...
Provide a guided developer workflow for adding a new adapter to the Daidala
...
board creation or `daidala_start`.
...
- a standalone Daidala dashboard, daemon, scheduler, MCP server, or second
  Kanban/state store;
...
- `daidala:setup` is officially bundled, versioned, loadable, and usable without
  the web dashboard.
...
- Live progress comes from Hermes Kanban and is never mirrored as Daidala state.
...
- a requested UI action lacks a documented public Hermes or existing Daidala
  service boundary;
