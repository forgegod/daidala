...
verified directory digest and one fenced YAML document; standalone and native
CLI dispatch remain equivalent; status exposes durable ledger identity and
provenance. The repository gate passed with 186 tests, Lefthook, Ruff, both pack
...
reject absent, wrong-board, wrong-card, stale, or wrongly assigned worker
contexts. The repository gate passed with 174 tests, Ruff, both pack validators,
sdist/wheel build, Twine, release-content validation, Markdown links, Lefthook,
and clean diff checks.

Phase 4 verdict: GREEN. Policy-aware card identity, bounded global and phase
projection, live worker-card validation, stale activation rejection, and the
worker handoff contract passed 81 focused tests; the repository gate passed
with 170 tests, Ruff, both pack validators, sdist/wheel build, Twine,
release-content validation, Markdown links, Lefthook, and clean diff checks.

Phase 3 verdict: GREEN. Append-only ledger identity, compare-and-swap recording,
approval invalidation, exclusive artifact creation, and read-back verification
passed 63 focused tests; the repository gate passed with 167 tests, Ruff, both
pack validators, sdist/wheel build, Twine, release-content validation, Markdown
links, Lefthook, and clean diff checks.
...
- treats frontmatter as descriptive source metadata and excludes it from the
  canonical constraint digest;
- rejects missing, multiple, unlabelled, or non-`yaml` fences and any prose
  outside the single fence;
