...
| 10 — operational hardening and release | Done | Preserve owned-worktree cleanup, restart idempotency, host-owned token accounting, the v0.18.2 compatibility matrix, release-content and dependency audits, and the Python 3.11/3.12 release gate. Gate: 21 Markdown files, 96 tests, Ruff, both pack validations, build, Twine, wheel inspection, isolated wheel installation, clean runtime dependency audit, workflow parse, and diff checks passed. |
