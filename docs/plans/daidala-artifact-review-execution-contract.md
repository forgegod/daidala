...
8. Restore through `archive_io.py` below `<resolved-data-root>/daidala/artifact-restores/<workflow-id>/<archive-id>/`; verify every digest; refuse conflicting bytes; publish with exclusive/atomic writes; update curator state only after the complete verified restore is available. Never write restored bytes into immutable historical ledger paths or mutate ledger identity.
...
1. Keep curation policy and scheduling disabled by default. Expose dry-run-first curator policy configuration through the shared Daidala CLI; apply requires the current curator state digest and literal desired policy digest. Schedule setup requires an operator preview and explicit confirmation that names the controller profile, interval, configured policy digest, and exact command/script identity.
...
3. Add an idempotent Daidala schedule preview/apply boundary that creates one profile-local script-only/no-agent Cron job invoking the installed Daidala curator apply path. The exact installed script is `$HERMES_HOME/scripts/daidala-artifact-curator.sh`, a credential-free launcher for `daidala curator tick`; Hermes runs it through Bash. Do not use a Python launcher whose import resolution depends on Hermes' host interpreter. The tick inherits the controller profile's resolved data root, produces no artifact content, and emits output only for transitions or classified failures.
