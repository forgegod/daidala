...
4. Extend `dashboard/dist/index.js` with an artifact panel that shows exact revision/digest/availability, renders text in a bounded escaped view, downloads binary/oversized evidence, distinguishes active from archived evidence, and never selects `latest` implicitly.
