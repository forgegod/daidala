...
1. Add a canonical revision-request artifact binding the source review/disposition preview, rejected implementation and verification identities, normalized human feedback, target plan revision, actor, and time. Store it before card or worktree mutation.
...
4. Extend plan-card identity so initial planning uses revision 0 and revision requests create one new `plan` card for revision N. Parent it to the source review card when present, assign the existing plan profile, pin the pack's plan skills, and include only revision-request references/digests plus the durable goal and definition—not raw logs.
...
7. Extend the shared parser/dispatcher with equivalent native `hermes daidala` and standalone `daidala` commands:
   - `review show WORKFLOW_ID` returns the current bounded review/disposition packet, exact tuple, allowed actions, and card IDs without mutation or arbitrary paths;
