...
**Status:** complete — Phase 0 and Phase 1 completed on `feat/dashboard-constraint-authoring`
...
| 1 | Add configuration verification panel | done (`577 passed`; `lefthook validate`; `ruff check .`; pack validation; dashboard compatibility probe) | The panel remains read-only and verifies persisted root, TTL, registration, checkout, and Projects-link state without exposing secrets/private destinations or supervising a workflow |
