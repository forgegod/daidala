...
| `repository_registration.py` | Preview-confirmed GitHub repository registration from a canonical remote and committed manifest, including path-free inspect classification (`needs-bootstrap` / `registerable` / `already-registered` / `blocked`), strict profile-local defaults, preview-confirm defaults check/write, and non-secret two-record apply. |
| `repository_bootstrap.py` | Preview-confirmed bootstrap of conservative committed `.daidala` policy via host `gh` Git Data API and one pull request on the inspected repository, plus a path-free pending PR receipt; never registers or updates the default branch. |
...
- Pack schema version 2 is the only accepted shape. Top-level `skills` owns each
  unique provider and expected digest; stage `skills` rows own only catalog name
  and activation mode. Installation, content, and readiness operate over the
  complete catalog; Kanban card assignment and activation manifests operate only
  over stage bindings. Do not add a version-1 compatibility reader.
...
- Pack skill actions recompute the complete action preview identity before
  ...
  only catalog-declared pack/skill identities, exact source links, and bounded `SKILL.md`
