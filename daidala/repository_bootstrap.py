...
from .projects import ProjectManifest, parse_project_manifest, parse_strict_yaml
...
BOOTSTRAP_RECEIPTS_SCHEMA = "daidala.repository-bootstrap-receipts/v1"
BOOTSTRAP_RECEIPTS_FILENAME = "repository-bootstrap-receipts.yaml"
MAX_BOOTSTRAP_RECEIPTS_BYTES = 64 * 1024
MAX_BOOTSTRAP_RECEIPTS = 128
...
class BootstrapReceipt:
    """Profile-local public follow-up links for one unpublished bootstrap branch."""
    ...
    default_branch: str
    commit_sha: str
    compare_pull_request: str
    pull_request: str | None = None

    def __post_init__(self) -> None:
        if not isinstance(self.repository_canonical, str) or not _REPOSITORY.fullmatch(
            self.repository_canonical
        ):
            raise RepositoryBootstrapError("bootstrap receipt repository is invalid")
        if not isinstance(self.project_id, str) or not _SLUG.fullmatch(self.project_id):
            raise RepositoryBootstrapError("bootstrap receipt project ID is invalid")
        if not isinstance(self.controller_profile, str) or not _SLUG.fullmatch(
            self.controller_profile
        ):
            raise RepositoryBootstrapError("bootstrap receipt profile is invalid")
        if (
            not isinstance(self.target_branch, str)
            or not isinstance(self.default_branch, str)
            or not _BRANCH.fullmatch(self.target_branch)
            or not _BRANCH.fullmatch(self.default_branch)
            or self.target_branch == self.default_branch
        ):
            raise RepositoryBootstrapError("bootstrap receipt branches are invalid")
        if (
            not isinstance(self.commit_sha, str)
            or not re.fullmatch(r"[0-9a-f]{40}", self.commit_sha)
        ):
            raise RepositoryBootstrapError("bootstrap receipt commit is invalid")
        _require_public_github_url(
            self.compare_pull_request, self.repository_canonical, "compare"
        )
        if self.pull_request is not None:
            _require_public_github_url(self.pull_request, self.repository_canonical, "pull")

    def open_url(self) -> str:
        return self.pull_request or self.compare_pull_request
    ...
    def to_public_dict(self) -> dict[str, object]:
        return {
            ...
            "target_branch": self.target_branch,
            "default_branch": self.default_branch,
            "compare_pull_request": self.compare_pull_request,
    ...
    @classmethod
    def from_result(cls, result: BootstrapResult) -> BootstrapReceipt:
        links = bootstrap_github_links(
            repository_canonical=result.preview.repository_canonical,
            target_branch=result.preview.target_branch,
            default_branch=result.preview.default_branch,
        )
        return cls(
            repository_canonical=result.preview.repository_canonical,
            project_id=result.preview.project_id,
            controller_profile=result.preview.controller_profile,
            target_branch=result.preview.target_branch,
            default_branch=result.preview.default_branch,
            commit_sha=result.commit_sha,
            compare_pull_request=str(links["compare_pull_request"]),
        )

    @classmethod
    def from_record(cls, raw: object) -> BootstrapReceipt:
        if not isinstance(raw, dict):
            raise RepositoryBootstrapError("bootstrap receipt must be an object")
        allowed = {
        ...
        if set(raw) - allowed or not allowed.issubset(set(raw)):
            raise RepositoryBootstrapError("bootstrap receipt fields are invalid")
        pull_request = raw["pull_request"]
        if pull_request is not None and not isinstance(pull_request, str):
            raise RepositoryBootstrapError("bootstrap receipt pull request is invalid")
        return cls(
            repository_canonical=str(raw["repository_canonical"]),
            project_id=str(raw["project_id"]),
            controller_profile=str(raw["controller_profile"]),
            target_branch=str(raw["target_branch"]),
            default_branch=str(raw["default_branch"]),
            commit_sha=str(raw["commit_sha"]),
            compare_pull_request=str(raw["compare_pull_request"]),
            pull_request=pull_request,
        )


class BootstrapReceiptStore:
    """One mode-0600 pending-bootstrap document for a controller profile."""

    def __init__(self, data_root: Path) -> None:
        if not isinstance(data_root, Path) or not data_root.is_absolute():
            raise RepositoryBootstrapError("bootstrap receipt root must be an absolute path")
        self.path = data_root.resolve() / BOOTSTRAP_RECEIPTS_FILENAME

    def read(self) -> tuple[BootstrapReceipt, ...]:
        try:
            content = read_private_text(
                self.path,
                maximum_bytes=MAX_BOOTSTRAP_RECEIPTS_BYTES,
                label="bootstrap receipts",
            )
        except FileNotFoundError:
            return ()
        except ProfileFileError as error:
            raise RepositoryBootstrapError(str(error)) from error
        return parse_bootstrap_receipts(content)

    def upsert(self, receipt: BootstrapReceipt) -> None:
        rows = [
            row
            for row in self.read()
            if row.repository_canonical != receipt.repository_canonical
        ]
        rows.append(receipt)
        self._replace(tuple(rows))

    def remove(self, repository_canonical: str) -> None:
        current = self.read()
        kept = tuple(row for row in current if row.repository_canonical != repository_canonical)
        ...
        if not kept:
            try:
                ...
                raise RepositoryBootstrapError("cannot remove bootstrap receipts") from error
            return
        self._replace(kept)

    def pending_for(
        self, *, exclude_repositories: set[str]
    ) -> tuple[BootstrapReceipt, ...]:
        return tuple(
            row
            for row in self.read()
            if row.repository_canonical not in exclude_repositories
        )

    def _replace(self, rows: tuple[BootstrapReceipt, ...]) -> None:
        if len(rows) > MAX_BOOTSTRAP_RECEIPTS:
            raise RepositoryBootstrapError("bootstrap receipt capacity exceeded")
        seen: set[str] = set()
        for row in rows:
            if row.repository_canonical in seen:
                raise RepositoryBootstrapError("bootstrap receipts contain a duplicate repository")
            seen.add(row.repository_canonical)
        payload = {
            "schema": BOOTSTRAP_RECEIPTS_SCHEMA,
            "receipts": [row.to_record() for row in rows],
        }
        canonical = yaml.safe_dump(
            payload, allow_unicode=True, default_flow_style=False, sort_keys=False
        )
        try:
            atomic_write_private_text(self.path, canonical, label="bootstrap receipts")
        except ProfileFileError as error:
            raise RepositoryBootstrapError(str(error)) from error


def parse_bootstrap_receipts(content: str) -> tuple[BootstrapReceipt, ...]:
    raw = parse_strict_yaml(
        content, label="bootstrap receipts", maximum_bytes=MAX_BOOTSTRAP_RECEIPTS_BYTES
    )
    if not isinstance(raw, dict) or set(raw) != {"schema", "receipts"}:
        raise RepositoryBootstrapError("bootstrap receipts schema is invalid")
    if raw["schema"] != BOOTSTRAP_RECEIPTS_SCHEMA:
        raise RepositoryBootstrapError("bootstrap receipts schema is invalid")
    receipts = raw["receipts"]
    if not isinstance(receipts, list) or len(receipts) > MAX_BOOTSTRAP_RECEIPTS:
        raise RepositoryBootstrapError("bootstrap receipts list is invalid")
    return tuple(BootstrapReceipt.from_record(row) for row in receipts)


def _require_public_github_url(value: object, canonical: str, kind: str) -> None:
    prefix = f"https://github.com/{canonical}/"
    if not isinstance(value, str) or not value.startswith(prefix) or any(
        ord(character) < 32 for character in value
    ):
        raise RepositoryBootstrapError(f"bootstrap receipt {kind} URL is invalid")
    if kind == "compare" and "/compare/" not in value:
        raise RepositoryBootstrapError("bootstrap receipt compare URL is invalid")
    if kind == "pull" and "/pull/" not in value:
        raise RepositoryBootstrapError("bootstrap receipt pull request URL is invalid")
    ...
    def apply(
        ...
        *,
        ...
        result = BootstrapResult(
            preview=preview, commit_sha=commit_sha, branch=preview.target_branch
        )
        BootstrapReceiptStore(self.data_root).upsert(BootstrapReceipt.from_result(result))
        return result
