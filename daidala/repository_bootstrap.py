...
BOOTSTRAP_PR_TITLE = "chore: bootstrap Daidala project policy"
BOOTSTRAP_PR_BODY = (
    "Adds conservative .daidala policy generated by Daidala bootstrap. "
    "Review and merge to enable registration."
)
...
class BootstrapPreview:
    ...
    def to_dict(self) -> dict[str, object]:
        ...
        return {
            ...
            "next_step": (
                "Merge the opened pull request on the inspected repository, "
...
class BootstrapResult:
    ...
    def to_dict(self) -> dict[str, object]:
        ...
        if self.pull_request is not None:
            payload["pull_request"] = self.pull_request
            links = payload.get("links")
            if isinstance(links, dict):
                payload["links"] = {**links, "pull_request": self.pull_request}
        return payload
...
class RepositoryBootstrapService:
    ...
    def apply(
        ...
        *,
        ...
        store = BootstrapReceiptStore(self.data_root)
        store.upsert(
            BootstrapReceipt.from_result(
                BootstrapResult(
                    preview=preview, commit_sha=commit_sha, branch=preview.target_branch
                )
            )
        )
        pull_request = self._open_pull_request(preview)
        result = BootstrapResult(
            preview=preview,
            commit_sha=commit_sha,
            branch=preview.target_branch,
            pull_request=pull_request,
        )
    ...
    def _open_pull_request(self, preview: BootstrapPreview) -> str:
        ...
        if (
            ...
            raise RepositoryBootstrapError(
                "GitHub pull request could not be created on the inspected repository"
            )
        url = output.strip().splitlines()[-1].strip() if output.strip() else ""
        _require_public_github_url(url, preview.repository_canonical, "pull")
        return url
