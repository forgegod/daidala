---
name: setup
description: Use when installing, checking, or starting Wingstaff for the first time. Guides explicit prerequisite discovery, previews the exact workflow request, and requires confirmation before mutation.
version: 0.1.0
author: Wingstaff
license: MIT
metadata:
...
Guide a user from an installed Wingstaff plugin to one explicitly confirmed
workflow start. Setup prepares and starts; `wingstaff:orchestrate` remains the
worker contract. This skill works without the web dashboard and uses the same
`wingstaff_start` request in dashboard and non-dashboard sessions.

## When to Use

Load this skill explicitly as `wingstaff:setup` when the user wants to install,
check, configure, or start Wingstaff. Do not use it for stage execution or for
resuming an already-created workflow.

## Procedure

1. Check the active Hermes version, Wingstaff plugin state, gateway or dispatcher
   ...
   `wingstaff_pack_info`; do not inspect private databases or infer names.
...
7. Build and display the exact request below. Validate required fields and show
   ...
   and constraint provenance. Do not call `wingstaff_start` yet.
8. Ask the user to explicitly confirm that exact preview. A general request to
   configure Wingstaff is not confirmation. If the preview changes, show it again
   and obtain fresh confirmation.
9. After confirmation, call `wingstaff_start` exactly once. Report the returned
...
10. Stop after start. Hermes Kanban dispatches workers using
    `wingstaff:orchestrate`; setup does not execute lifecycle stages.

## Exact Start Request

Use only the current `wingstaff_start` argument names:
...
- Using `--profile` as Wingstaff's default-profile input; it is a Hermes host
...
- Copying pack lifecycle data instead of calling `wingstaff_pack_info`.

## Verification Checklist

- [ ] Supported Hermes and enabled Wingstaff plugin checked.
...
- [ ] Preview contains the complete current `wingstaff_start` request.
- [ ] Human explicitly confirmed the unchanged preview.
- [ ] `wingstaff_start` called exactly once after confirmation.
