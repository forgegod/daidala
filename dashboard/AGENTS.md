...
- Workflow polling is read-only. Mutations are limited to confirmed declared
  ...
  keeps that public PR URL in the inventory receipt. A successful apply
  clears the Bootstrap preview panel; the success message and the row's
  receipt-projected pending PR link are the feedback that it worked. A failed
  apply keeps the panel so the still-valid preview digest can be retried.
  Apply
